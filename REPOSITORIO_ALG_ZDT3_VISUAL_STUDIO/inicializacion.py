...
def crea_vector_landa(): #creación vector landa para cada subproblema (N individuos)
    ...
    for i in range(0,main.N):
        vector_landa.append([main.max_ - ((i)/(main.N-1)),main.min_ + ((i)/(main.N-1))])
    return vector_landa

'''
def vecinos_landa_mas_cercanos_1():
    vecinos_landa_global = []
    for i in range(0,_main.N):
        vecinos_landa_i = []
        vecinos_landa_i.append(i)
        for j in range(1,int(_main.T)):
            if(vecinos_landa_i.__len__()==_main.T):exit
            if(i-j>=0): vecinos_landa_i.append(i-j) 
            if(i+j<=_main.N): vecinos_landa_i.append(j+i)
        vecinos_landa_global.append(vecinos_landa_i)
    #print (vecinos_landa_global.__len__())
    #print (vecinos_landa_global[0].__len__())
    return vecinos_landa_global
'''
...
def vecinos_landa_mas_cercanos(vector_landa): #devuelve una lista que engloba los T vecinos mas cercanos para cada vecotr landa i
    ...
    for i in range(0,main.N):
        ...
        for j in range(0,main.N): #bucle para calcular todas las distancias entre los vectores de forma euclídea
...
def vecindad(vecinos_cercanos_i):
    l = list(sorted(enumerate(vecinos_cercanos_i), key=lambda x: x[1])[0:main.T]) #creo una lista enumerada ordenada con los T vecinos
    res = []
    for i in range(0,main.T):
...
def crea_vector_random():
    ...
    for i in range(0,main.N):    
        res_aux = []
        for j in range(0,main.D):
            res_aux.append(random.random())
        res.append(res_aux)
    return res
...
def inicializacion():
    ...
    for i in range(0,main.N):
        evaluaciones.append(ZDT3.func(random_vector[i])) #añado a evaluaciones, el valor de obj[0](f1) y obj[1](f2)
...
''' FIN INICIALIZACIÓN  ''' 
    ...
    #inicializacion()
    #res = vecinos_landa_mas_cercanos_1()
    #print(res)
    #res = crea_vector_landa()
    #print(res)
